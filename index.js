...
function processProduct(num1, num2, callback) {
  const product = function multiplty(num1, num2){
    num1 * num2;
  return num1 * num2;
  }
...
function getFullNames(runners) {
  ...
  runners.forEach(function(currentValue, index){
    return newArray.push(`${currentValue.last_name}, ${currentValue.first_name}`);
  });
  return newArray;
}
...
function firstNamesAllCaps(runners) {
  let newArray = [];
  runners.map(function(currentValue, index){
    return newArray.push(`${currentValue.first_name.toUpperCase()}`);
  });
  return newArray;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let newArray = runners.filter(function(currentValue){
